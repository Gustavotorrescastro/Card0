  ...
  return(
    <Page>
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-md border border-edenred-border mt-10">
        ...
        <p className="text-edenred-textSecondary mb-8 text-center">Descubra quanto impacto ambiental foi evitado desde a adesão aos pagamentos digitais.</p>
        <form onSubmit={handleCalculate} className="space-y-6">
          <div>
            <label htmlFor="userId" className="block text-sm font-medium text-edenred-text mb-2">ID do Usuário</label>
            <input type="text" id="userId" value={userId} onChange={(e)=>setUserId(e.target.value)} required className="w-full px-4 py-2 border border-edenred-border rounded-md focus:outline-none focus:ring-2 focus:ring-edenred-primary focus:border-transparent" placeholder="Ex: 12345"/>
          </div>
          <div>
            <label htmlFor="startDate" className="block text-sm font-medium text-edenred-text mb-2">Data de Adesão (Digital)</label>
            <input type="date" id="startDate" value={startDate} onChange={(e)=>setStartDate(e.target.value)} required className="w-full px-4 py-2 border border-edenred-border rounded-md focus:outline-none focus:ring-2 focus:ring-edenred-primary focus:border-transparent"/>
          </div>
          <button type="submit" disabled={loading} className="w-full bg-edenred-primary text-white font-bold py-3 rounded-md hover:bg-edenred-secondary transition-colors disabled:opacity-50">
